...
type AttributeService interface {
	GetAllAttributes(ctx context.Context) ([]model.AttributeDTO, error)
}
...
func (s *AttributeServiceImpl) GetAllAttributes(ctx context.Context) ([]model.AttributeDTO, error) {
	// データベースから属性を取得
	...
	// 属性をDTOに変換
	dtos := util.ConvertSlice(attributes, func(attr *ent.Attribute) model.AttributeDTO {
		return model.AttributeDTO{
			ID:   attr.ID,
