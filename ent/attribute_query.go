...
func (aq *AttributeQuery) loadServants(ctx context.Context, query *ServantQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *Servant)) error {
	...
	for _, n := range neighbors {
		fk := n.attribute_servants
		if fk == nil {
			return fmt.Errorf(`foreign-key "attribute_servants" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "attribute_servants" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
