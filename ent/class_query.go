...
func (cq *ClassQuery) QueryServants() *ServantQuery {
	query := (&ServantClient{config: cq.config}).Query()
...
func (cq *ClassQuery) Clone() *ClassQuery {
	...
	return &ClassQuery{
		...
		withServants: cq.withServants.Clone(),
...
func (cq *ClassQuery) WithServants(opts ...func(*ServantQuery)) *ClassQuery {
	query := (&ServantClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withServants = query
	return cq
}
...
func (cq *ClassQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Class, error) {
	...
	if query := cq.withServants; query != nil {
		if err := cq.loadServants(ctx, query, nodes,
			func(n *Class) { n.Edges.Servants = []*Servant{} },
			func(n *Class, e *Servant) { n.Edges.Servants = append(n.Edges.Servants, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ClassQuery) loadServants(ctx context.Context, query *ServantQuery, nodes []*Class, init func(*Class), assign func(*Class, *Servant)) error {
