...
func (tq *TraitQuery) QueryServants() *ServantQuery {
	query := (&ServantClient{config: tq.config}).Query()
...
func (tq *TraitQuery) Clone() *TraitQuery {
	...
	return &TraitQuery{
		...
		withServants: tq.withServants.Clone(),
...
func (tq *TraitQuery) WithServants(opts ...func(*ServantQuery)) *TraitQuery {
	query := (&ServantClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withServants = query
	return tq
}
...
func (tq *TraitQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Trait, error) {
	...
	if query := tq.withServants; query != nil {
		if err := tq.loadServants(ctx, query, nodes,
			func(n *Trait) { n.Edges.Servants = []*Servant{} },
			func(n *Trait, e *Servant) { n.Edges.Servants = append(n.Edges.Servants, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TraitQuery) loadServants(ctx context.Context, query *ServantQuery, nodes []*Trait, init func(*Trait), assign func(*Trait, *Servant)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(trait.ServantsTable)
		s.Join(joinT).On(s.C(servant.FieldID), joinT.C(trait.ServantsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(trait.ServantsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(trait.ServantsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Servant](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "servants" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
